...
unsigned char
	*Seg;				// Allocated segment (converted to buffer for linear address space
unsigned
...
unsigned peek(unsigned char *seg, unsigned offset)
{
	return seg[offset];
}

unsigned peekw(unsigned char *seg, unsigned offset)
{
	return seg[offset] + (seg[offset + 1] << 8);
}

void poke(unsigned char *seg, unsigned offset, unsigned char val)
{
	seg[offset] = val;
}

unsigned char *alloc_seg(unsigned size)
{
	return (unsigned char *)malloc(size * 16);
}
...
//	IOB_size = 4096;
	...
	for(t = 0; t < Dheader.Tracks; ++t) {
		for(i=0; i < Dheader.Tsize; ++i) {
			...
			poke(Seg, i, c); }
		Nsec = so = 0;
		while((Nsec < 64) && (s = peekw(Seg, so))) {	// fix there are only up to 64 IDAMs
			so += 2;
