...
bool parseRecipe(char *line, recipe_t *recipe) {
    ...
    if (!isIsisName(recipe->iname)) {
        warn("%s: %s", recipe->iname == iname ? "Cannot generate ISIS name" : "Invalid ISIS name", line);
        return false;
    }
    if (*recipe->loc == '^') {
        recipe->inRepo = true;
        ...
        recipe->inRepo = false;
    int locLen = (int)strlen(recipe->loc);
    if (locLen < 10)					// loc may be replaces with iname, make sure we allow 10 chars for it
        locLen = 10;
    int lineLen =
        (int)(strlen(recipe->iname) + strlen(recipe->attrib) + strlen(recipe->key) + locLen + 4);
    if (lineLen >= MAXLINE) {
        warn("Expanded line too long: %s", line);
        return false;
    }
...
bool updateRecipe(recipe_t *r) {
    ...
    if (r->inRepo) {
        ...
        if (!lp) {
            ...
            warn("Invalid(key, loc) pair: %s, ^%s", r->key, r->loc);
            r->inRepo = false;
            strcpy(r->loc, "*");		// force iname behaviour below
        }
    }
    ...
    if (!keyExists(r->key)) {
        if (*r->key != '*') {							// don't update if recipe signalled error
            ...
            case -1:
                warn("No match to key: %s or location: %s", r->key, r->loc);
