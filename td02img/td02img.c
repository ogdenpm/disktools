...
struct td0_header {
    uint16 	Sig;		// TD Signature
    uint8	Sequence;	// Volume sequence number
    uint8	Checksig;	// Check signature for multi-volume sets
    uint8	TDversion;	// Teledisk version
    uint8	Datarate;	// Data rate
    uint8	Drivetype;	// Source drive type
    uint8	Stepping;	// Stepping type
    uint8	DOSmode;	// Read according to DOS allocation
    uint8	Sides;		// # of sizes read
    uint16  Hcrc;				// Header crc
} Header;
#pragma pack(pop)
...
typedef struct {
    uint16	crc;					// comment block crc
    uint16	len;					// comment block length
    uint8	y, mo, d, h, mi, s;		// date
} comment_t;

// Individual track header
struct track_header {
    uint8	Tsectors;	// number sectors/track
    uint8	Tcylinder;	// Physical cylinder
    uint8	Tside;		// Physical side
    uint8	Tcrc;		// Crc of header
} Thead;
#pragma pack(pop)
...
struct sector_header {
    uint8	Scylinder;	// Cylinder number in ID field
    uint8	Sside;		// Side number in ID field
    uint8	Ssector;	// Sector number in ID field
    uint8	Ssize;		// Size of sector
    uint8	Sflags;		// Sector control flags
    uint8	Scrc;		// Sector header CRC
} Shead;
#pragma pack(pop)
...
struct data_header {
    uint16	Doffset;	// Offset to next data block
    uint8	Dmethod;	// Method of compression
} Dhead;
#pragma pack(pop)
...
void Dprintf(char *fmt, ...)
{
    ...
    if (Dlow <= Thead.Tcylinder && Thead.Tcylinder <= Dhigh)
        vprintf(fmt, args);
...
void dump(void *vp, unsigned char *t, unsigned char Mask)
{
    ...
    if((Debug & Mask) && Dlow <= Thead.Tcylinder && Thead.Tcylinder <= Dhigh)
        for(;;)
            switch(c = *t++) {
            case 0 : return;
            case 0x80 : printf("%02X", *p++);					continue;
            case 0x81 : printf("%04X", *(unsigned short *)p); p += 2;	continue;
            case 0x82 : printf("%d", *p++);					continue;
            case 0x83 : printf("%d", *(unsigned short *)p);	p += 2;	continue;
            default: putchar(c); 
            }
}
...
{
    unsigned short c, i, j, nl;
    comment_t comment;
    
    getblock(&comment, sizeof(comment), "comment header");


    c = compute_crc(&comment.len, sizeof(comment) - sizeof(comment.crc), j = nl = 0);

    printf(Cmap, " %u/%02u/%04u %u:%02u:%02u\n", comment.d, comment.mo+1, comment.y+1900, comment.h, comment.mi, comment.s);

    while(j++ < comment.len) {				// Process each character
        i = getbyte();
...
Opts:	/Dx,l,h	- enable Debug options\n\
    ...
    O=file	- specify Output filename			[filename.img]\n\
...
void main(int argc, char *argv[])
{
    ...
    while(!getblock(&Thead, sizeof(Thead), 0)) {
        ...
        if (Thead.Tsectors == 0) {
            printf("No sectors for track %d, side %d\n", Thead.Tcylinder, Thead.Tsectors);
        }
        ...
        for(s=0; s < Thead.Tsectors; ++s)
            process_sector();
        if(!issame(Smap, (byte)Size = Smap[0], Stop))	// 765 cannot do mixed sector sizes
