...
static int getData(int marker, uint16_t *buf, int length) {
    ...
    int i = 0;
    bool isGood;
    if (curFormat->options == O_ZDS ||
        curFormat->options == O_MTECH) // ZDS includes track and sector in CRC check
        buf[i++] = marker >> 8;
    buf[i++] = marker & 0xff;
    for (; i < length; i++) {
        if ((val = getByte()) < 0)
...
static uint8_t conflicts[32] = { 0,   64,  32,  255, 16,  255, 48, 255, 8,   72, 40,
                                 255, 24,  255, 56,  255, 4,   68, 36,  255, 20, 255,
                                 52,  255, 12,  76,  44,  255, 28, 255, 60,  255 };
...
static uint16_t hs8Sync(unsigned cylinder, unsigned slot) {
    ...
    if (curFormat->options == 0 && conflicts[slot] == cylinder && matchType == LSI_SECTOR &&
        getByteCnt(fromTs) > 12)
        matchType = matchPattern(2);
    return matchType;
}

static uint8_t lsiInterleave[] = { 0,  11, 22, 1,  12, 23, 2,  13, 24, 3,  14, 25, 4,  15, 26, 5,
        ...
        for (int i = 0; (slot = seekIndex(i)) != EODATA; i++) {
            ...
            if (!sectorStatus[slot] || (profile == 0 && (debug & D_NOOPTIMISE))) {
                ...
                if (matchType == NSI_SECTOR) {
                    ...
                    if (readCylinder >= 77)
                    ...
                    else if ((result = getData((readCylinder << 8) + slot, rawData,
                                               sectorSize + 13)) < 0)
                        ...
                        if (result == 1) {
                            if (cylinder != readCylinder) {
                                logFull(D_DECODER,
                                        "cylinder %d expecting %d, assuming seek error\n",
...
static void hs8GetTrack(int cylinder, int side) {
    ...
    for (int profile = 0; !done; profile++) {
        for (int i = 0; (slot = seekIndex(i)) != EODATA; i++) {
            if (slot < 0)
                continue;
            fromTs = peekTs();
            if (sectorStatus[slot] && profile != 0 &&
                !(debug & D_NOOPTIMISE)) // skip known good sectors unless D_NOOPTIMISE specified
                continue;
            ...
            if ((matchType = hs8Sync(cylinder, slot))) {
                ...
                if (curFormat->options == O_LSI &&
                    matchType == ZDS_SECTOR) { // LSI & ZDS on same track assume all ZDS
                ...
                DBGLOG(D_DECODER, "@%d-%d %s sector %d%s\n", dataPos, getByteCnt(fromTs),
                       matchType == LSI_SECTOR ? "LSI" : "ZDS", slot,
                       result == 1 ? "" : " bad crc");
                sectorStatus[slot] |= result;
            } else
                logFull(D_DECODER, "cannot find start of sector %d\n", slot);
    ...
    for (int slot = 0; slot < cntSlot; slot++) {
        idam.sectorId =
            (curFormat->options != O_NSI) ? slot : (lsiInterleave[slot] + cylinder * 8) % 32;
        addIdam(-slot, &idam);
    }
    setOnIndex(NULL);
    finaliseTrack();
            ...
            while (!restart && (matchType = matchPattern(1200))) {
                ...
                case TI_IDAM:
                    ...
                    if (result == 1) {
                        ...
                        logFull(D_DECODER, "TI IDAM @%d - head=%d, cyl=%d, sector=%d\n", idamPos,
                                (rawData[1] & 0xff) >> 3, rawData[2] & 0x7f, rawData[4] & 0xff);
                    } else
                        logFull(D_DECODER,
                                "TI IDAM @%d (crc error) - %02X % 02X % 02X % 02X % 02X % 02X\n",
                                idamPos, rawData[1] & 0xff, rawData[2] & 0xff, rawData[3] & 0xff,
                                rawData[4] & 0xff, rawData[5] & 0xff, rawData[6] & 0xff);
                ...
                case HP_IDAM:
                    ...
                    if (result == 1) { // valid so ok
                        ...
                        if (chkSizeChange(sSize) &&
                            savedData) { // new size but we already saved data!!
                        ...
                        DBGLOG(D_DECODER, "@%d-%d idam %d/%d/%d sSize %d\n", idamPos,
                               getByteCnt(fromTs), idam.cylinder, idam.side, idam.sectorId, sSize);
                        addIdam(idamPos, &idam);
                    } else
                        logFull(D_DECODER, "@%d idam %s\n", idamPos,
                                result < 0 ? "premature end\n" : "bad crc\n");
                    break;
                case TI_DATAAM:
                case DATAAM:
                case M2FM_DATAAM:
                case HP_DATAAM:
                    ...
                    if (result >= 0) {
                        ...
                        addSectorData(dataPos, result, sectorLen + 2,
                                      rawData + 1); // add data after address mark
                        savedData = true;
                        DBGLOG(D_DECODER, "@%d-%d data len %d%s\n", dataPos, getByteCnt(fromTs),
                               sectorLen, result == 1 ? "" : " bad crc");
                    } else
...
static void ssDumpTrack(char *usrfmt) {
    ...
    while ((matchType = matchPattern2(lock)) >= 0) {
        ...
        case TI_IDAM:
            ...
            if (result == 1) {
                ...
                printf("%d: TI IDAM - head=%d, cyl=%d, sector=%d - %d\n", idamPos,
                       (rawData[1] & 0xff) >> 3, rawData[2] & 0x7f, rawData[4] & 0xff, cellSize);
            } else
                printf("%d: TI IDAM (crc error) - %02X %02X %02X %02X %02X %02X - %d\n", idamPos,
                       rawData[1] & 0xff, rawData[2] & 0xff, rawData[3] & 0xff, rawData[4] & 0xff,
                       rawData[5] & 0xff, rawData[6] & 0xff, cellSize);
            break;
        case IDAM:
        case M2FM_IDAM:
        case HP_IDAM:
            ...
            if (result == 1) { // valid so ok
                ...
                printf("%d-%d: idam %d/%d/%d sSize %d - %d\n", idamPos, getByteCnt(fromTs),
                       idam.cylinder, idam.side, idam.sectorId, sSize, cellSize);
            } else
        ...
        case HP_DATAAM:
            ...
            result    = getData(matchType, rawData, sectorLen + 3);
            if (result >= 0) {
                printf("%d-%d: data len %d%s - %d\n", dataPos, getByteCnt(0), sectorLen,
                       result == 1 ? "" : " bad crc", cellSize);
            } else
