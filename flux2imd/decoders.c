...
#define FMBYTE_US   32      // 32uS per byte
...
bool chkSizeChange(uint8_t sSize);
void chkSptChange(uint16_t pos, bool isIdam);


// there is a potential problem in that LSI start byte can match the first byte of ZDS start
// in particular the LSI track / ZDS sector conflicts are
...
static uint16_t hsSync(uint8_t track, uint8_t slot) {
    int matchType;
    makeHSPatterns(track, slot);

    matchType = matchPattern(30);
    if (curFormat->encoding == E_FM8H && conflicts[slot] == track && matchType == LSI_SECTOR && getByteCnt() > 10)
...
void hsGetTrack(int track) {
    int slot;
    uint8_t sectorToSlot[32] = { 0 };
    char *fmt = "FM8H";
    char *savedFmt;
    uint16_t dataPos;
    uint16_t dataTrackPos;
    int matchType;
    ...
    for (int slot = 0; slot < cntSlot; slot++) {
        idam_t idam = { track, 0, slot, 0 };
        addIdam(slot * curFormat->spacing + HSIDAM, &idam);
    }

    for (int i = 0; (slot = seekBlock(i)) >= 0; i++) {
        // skip if already have good copy
        if (sectorToSlot[slot])
            continue;

        uint16_t rawData[138];      // sector + track + 128 bytes + 4 links + 2 CRC + 2 postamble
        int result;

        retrain(0);
        resetByteCnt();

        if (matchType = hsSync(track, slot)) {
            dataPos = getByteCnt();
            if (curFormat->options == O_LSI && matchType == ZDS_SECTOR) {    // LSI & ZDS on same track assume all ZDS
                    logFull(DECODER, "@%d ZDS sector after LSI sector, rescan assuming ZDS\n", dataTrackPos);
                    curFormat = lookupFormat("FM8H-ZDS");
                    initTrack(track, 0);    // clear any data collected so far assume all ZDS
            ...
            if (matchType == LSI_SECTOR) {
                ...
                if ((result = getData((track << 8) + slot + 0x80, rawData, 138)) < 0) {
        ...
        if (debug & DECODER) {
            ...
            while ((val = getByte()) >= 0)
                printf(" %02X", val);
            logFull(DECODER, "@%d end of sector %d\n", getByteCnt());
        }
...
void ssGetTrack(uint8_t track, uint8_t side) {
    ...
    initTrack(track, side);
    uint8_t sSize = curFormat->sSize;
    ...
    for (int i = 1; !done && (seekBlock(i) >= 0); i++) {
        ...
        while (!restart && (matchType = matchPattern(1200))) {
            ...
            case HP_IDAM:
                ...
                if (result == 1) {           // valid so ok
                    ...
                    if (chkSizeChange(sSize) && savedData) {    // new size but we already saved data!!
                        logFull(DECODER, "@%d restarted with new size\n", idamPos);
                        restart = true;
                        break;
                    }
                    //chkSptChange(idamPos, true);

                    logFull(DECODER, "@%d-%d idam %d/%d/%d sSize %d\n", idamPos, getByteCnt(), idam.track, idam.side, idam.sectorId, sSize);
            ...
            case HP_DATAAM:
                ...
                if (result >= 0) {
                    ...
                    savedData = true;
                    logFull(DECODER, "@%d-%d data len %d%s\n", dataPos, getByteCnt(), sectorLen, result == 1 ? "" : " bad crc");
                } else
                    logFull(DECODER, "@%d data premature end\n", dataPos);
                break;
            case DELETEDAM:
            case M2FM_DELETEDAM:
            case HP_DELETEDAM:
        ...
        if (restart) {
            ...
            logFull(DECODER, "@%d end of track\n", getByteCnt());
            done = checkTrack();
        }
    }
    finaliseTrack();
...
bool chkSizeChange(uint8_t sSize) {
    uint8_t curSSize = curFormat->sSize;
